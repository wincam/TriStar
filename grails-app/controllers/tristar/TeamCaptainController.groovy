...
class TeamCaptainController extends RestfulController {
    ...
    def delete() {
        // checks params
        if (params.teamID == null || params.userID == null){
            render (status: 400, text: "params missing fields")
            return
        }
        ...
        User captainToRemove = User.findByUsername(params.userID.toString())

        // check if user is valid
        if (captainToRemove == null){
            render (status: 404, text: "${params.userID.toString()} does not exist")
            return
        }

        // check if user is a captain
        if (!team.getCaptains().contains(captainToRemove)){
            render (status: 404, text: "${params.userID.toString()} not a captain in ${params.teamID.toString()}")
            return
        }

        team.removeFromCaptains(captainToRemove)
