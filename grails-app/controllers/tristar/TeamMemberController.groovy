...
class TeamMemberController extends RestfulController {
    ...
    def delete() {
        ...
        User memberToRemove = User.findByUsername(params.userID.toString())
        ...
        if (!team.getMembers().contains(memberToRemove)){
            render (status: 404, text: "${params.userID.toString()} not a member in ${params.teamID.toString()}")
            return
        }

        team.removeFromMembers(memberToRemove)
